...
Do **not** use `echo '...' >> ledger.jsonl` — single-quoted shell strings break on any field value containing a `'` character and permit shell injection. The helper uses `json.dumps` internally and is safe for free-form text.
