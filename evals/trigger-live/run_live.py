...
def load_cases(only_skill: str | None) -> list[tuple[str, str]]:
    """Return (expected_skill, prompt) pairs from all should_fire cases."""
...
def main() -> int:
    ...
    pairs = load_cases(args.skill)
    if not args.all:
        seen: set[str] = set()
        sampled = []
        for skill, prompt in pairs:
            if skill not in seen:
                sampled.append((skill, prompt))
                seen.add(skill)
        pairs = sampled
    ...
    for skill, prompt in pairs:
        ...
        print(f"  {icon} expected={skill:<26} fired={fired}")
    ...
    print(
        f"\nRouting accuracy: exact {exact}/{total}, router-tolerant {tolerant}/{total}"
    )
    print(f"Results: {out_path}")
    misroutes = [r for r in rows if not r["router_tolerant"]]
    if misroutes:
